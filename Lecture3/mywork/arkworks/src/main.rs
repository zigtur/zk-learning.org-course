...
fn check_puzzle_matches_solution<const N: usize, ConstraintF: PrimeField>(
    ...
    for (p_row, s_row) in puzzle.0.iter().zip(&solution.0) {
        for (p_cell, s_cell) in p_row.iter().zip(s_row) {
            //Ensure that the solution `s` is in the range [1, N]
            s_cell.is_leq(&UInt8::constant(N as u8))?
                .and(&s_cell.is_geq(&UInt8::constant(1))?)?
                .enforce_equal(&Boolean::TRUE)?;

            //Ensure that either the puzzle slot is 0, or 
            //that the slot matches equivalent slot in the solution
            (p_cell.is_eq(s_cell)?.or(&p_cell.is_eq(&UInt8::constant(0))?)?)
                .enforce_equal(&Boolean::TRUE);
        }
